...
<head>
	...
	<link rel="stylesheet" href="<?php echo base_url(); ?>public/css/order_view.css?v=<?php echo time(); ?>" type="text/css">
					...
					<?php foreach ($sale_order as $key => $val) {
						if ($val['status'] != 3) {
							...
							<tr>
								...
								<td><?php echo number_format($val['total'], 0, '', '.') . ' VND'; ?></td>
								...
								<td>
									<?php if ($val['status'] == 1) { ?>
										<a href="<?php echo base_url('Order/del_order/' . $val['order_id']) ?>" onclick="return confirm('Bạn muốn xóa đơn hàng này ? ')"><i class="fas fa-trash-alt" style="color : red; font-size : 20px; margin-right : 10px"></i></a>
									<?php } ?>
